...
class DPODataset_action(Dataset):
    def __init__(self, item_ids, score_list, action_list, item_content_path, dataset_name, top_k=1):
        """
        Args:
        ...
        """
        self.item_ids = torch.cat(item_ids)
        self.top_k = top_k
        ...
        self.scores_tensor = torch.cat(score_list,dim=1)  # (k, n)
        # self.probs_tensor = torch.cat(prob_list,dim=1)   # (k, n)
        # self.embedding_list = torch.cat(embedding_list,dim=1)
        self.action_list = torch.cat(action_list,dim=1)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "pos_actions": self.action_list[chosen_idx][item_idx],
            "neg_actions": self.action_list[rejected_idx][item_idx],
        }
    ...
    def remove_uniform_rows(self):
        ...
        self.action_list = self.action_list[:, removed_mask, :]
...
class DPOCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        batch = {
            ...
            "pos_actions": torch.stack([f['pos_actions'].cpu() for f in features], dim=0),
            "neg_actions": torch.stack([f['neg_actions'].cpu() for f in features], dim=0),
        }
