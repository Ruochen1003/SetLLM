...
class FilterLLM(PreTrainedModel):
    def __init__(self, config, encoder, num_users, num_items):
        ...
        for name, param in self.named_parameters():
            if ("item_embeddings" not in name) and ("mapper" not in name) and ('lora' not in name):
                # if ("mapper" not in name):
                param.requires_grad = False

        # Tie the weights between the user embeddings and the user recommendation head

    def _compute_item_head_weight(self):
        origin_item_emb = self.item_embeddings.weight
        return origin_item_emb
    ...
    def item_head(self, input):
        linear_matrix = self._compute_item_head_weight()
        logist = torch.matmul(input, linear_matrix.T)
        return logist


    def forward(self, user_id, input_ids_prompt, attention_mask, labels = None, **kwargs):
        # Prompt embedding
        ...
        user_logist = self.item_head(last_token_hidden_states)

        multi_item_label = self.get_multi_hot_label(labels)
        n = multi_item_label.sum(dim=-1, keepdim=True)
        #align_loss = torch.mean((last_token_hidden_states - item_emb)**2)

        interaction_loss = F.cross_entropy(user_logist, multi_item_label.float()/n)
        loss = interaction_loss #+ self.lamda * align_loss
        _,topk_item = torch.topk(user_logist, 20)

        return {'loss':loss, 'logits':user_logist, 'topk_item':topk_item, 'user_id': user_id}
    ...
    def get_multi_hot_label(self, labels):
        labels = labels + 1
        multi_label = torch.zeros((len(labels), self.num_items + 1), dtype=torch.bool).to(labels.device)
